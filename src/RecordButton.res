...
let make = (
  ...
  let renderButtons = () => {
    let hasTooManyDistinctEvents = eventCount > 2
    ...
    if hasTooManyDistinctEvents {
      <>
        ...
        <div className="card-actions">
          <Components.Button className="btn-primary" onClick={_ => setEvents(_ => [])}>
            {React.string("Start over")}
          </Components.Button>
          <Components.Button className="btn-error" onClick={_ => onSave(events)}>
      ...
      <div className="card-actions">
        <Components.Button className="btn-outline" onClick={_ => setEvents(_ => [])}>
          {React.string("Start over")}
        </Components.Button>
        <Components.Button className="btn-primary" onClick={_ => onSave(events)}>
    ...
    <div className="card-body">
      ...
      <Components.Text>
        <>
          ...
          <strong> {React.string(Hid.buttonLabel(buttonId))} </strong>
          {React.string(" button for at least 2 seconds, then release it. The signals it sends will appear below.")}
        </>
      </Components.Text>
      <div className="mockup-code max-h-48 overflow-auto text-xs">
        {displayEvents->Array.length == 0
          ? <Html.Pre dataPrefix="…"> <code> {React.string("Listening for signals…")} </code> </Html.Pre>
          : React.array(
              displayEvents->Array.mapWithIndex(((event, hex), index) =>
                <Html.Pre key={Int.toString(index)} dataPrefix=">">
                  <code>
                    {React.string(
                      Date.fromTime(startTime +. event->WebHid.timeStamp)->Date.toLocaleTimeString ++
                      " — " ++ hex,
                    )}
                  </code>
                </Html.Pre>
